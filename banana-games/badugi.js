...
function createTree(hand, deadCards, drawsRemaining, maxDraw) {
  ...
  if (drawsRemaining > 1) {
    for (const node of root.children) {
      let childrenMaxDraws = 2
      if (node.lastAction === 'draw1' || node.lastAction === 'draw0') {
        childrenMaxDraws = 1;
      }
      node.children = createTree(node.hand, node.deadCards, drawsRemaining - 1, childrenMaxDraws).children;
    }
  }
  return root;
} 

function scoreNodes(node) {
  ...
  for (const action of Object.keys(actionToChildScores)) {
    ...
    if (node.lastAction === '') {
      console.log(`if new dead cards ${action}, avg node score is`, averageScore)
    }
  ...
  node.comment = `New dead cards should be ${bestActionIdentifier}`;
}
